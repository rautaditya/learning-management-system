...
const AddInstructor = () => {
    ...
    const handleEdit = (instructor) => {
        setFormData({
            ...
            dateJoined: instructor.dateJoined?.split("T")[0] || ''
        });
        setIsEditing(true);
        setEditId(instructor._id);
    };
    ...
    return (
        <div className="max-w-5xl mx-auto mt-12 p-5 border border-gray-300 rounded-lg">
            <h2 className="text-2xl font-bold mb-4">
                {isEditing ? 'Edit Instructor' : 'Add Instructor'}
            </h2>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="fullName" value={formData.fullName} onChange={handleChange} required className="w-full px-3 py-2 border rounded-md" />
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="department" value={formData.department} onChange={handleChange} required className="w-full px-3 py-2 border rounded-md" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Phone Number</label>
                    <input type="text" name="phoneNumber" value={formData.phoneNumber} onChange={handleChange} className="w-full px-3 py-2 border rounded-md" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Education</label>
                    <input type="text" name="education" value={formData.education} onChange={handleChange} className="w-full px-3 py-2 border rounded-md" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Date Joined</label>
                    <input type="date" name="dateJoined" value={formData.dateJoined} onChange={handleChange} className="w-full px-3 py-2 border rounded-md" />
                </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full border border-gray-300">
                            ...
                            <tbody>
                                {instructors.map((inst) => (
                                    <tr key={inst._id} className="hover:bg-gray-50">
                                        <td className="border px-4 py-2">{inst.fullName}</td>
                                        <td className="border px-4 py-2">{inst.email}</td>
                                        <td className="border px-4 py-2">{inst.department}</td>
                                        <td className="border px-4 py-2">{inst.phoneNumber}</td>
                                        <td className="border px-4 py-2">{inst.education}</td>
                                        <td className="border px-4 py-2">{inst.dateJoined?.split("T")[0]}</td>
                                        <td className="border px-4 py-2">
                                            <button onClick={() => handleEdit(inst)} className="mr-2 px-3 py-1 bg-yellow-500 text-white rounded">Edit</button>
                                            <button onClick={() => handleDelete(inst._id)} className="px-3 py-1 bg-red-600 text-white rounded">Delete</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
